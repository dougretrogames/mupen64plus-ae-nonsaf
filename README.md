...
10. **Configuração Inicial Assistida (Primeira Abertura)**: Implementação de um assistente de configuração automática que aparece apenas na primeira inicialização. O usuário pode escolher a proporção de tela (**4:3** ou **16:9**) e o perfil de emulação Glide64 (**Fast/Velocidade** ou **Accurate/Qualidade**). Ao selecionar o perfil *Fast*, o sistema ajusta automaticamente a resolução para **480x360** para otimizar a performance em hardware limitado. Após a configuração, o app exibe uma mensagem de conclusão e encerra o processo para garantir que as alterações sejam aplicadas de forma limpa.
11. **Estabilidade com Frontends**: Restauração do modo de lançamento (`launchMode`) original e correção da lógica de monitoramento do processo de emulação. Isso resolve problemas de fechamento repentino ou falha ao trocar de jogo quando o emulador é iniciado por frontends externos (como ARC Browser, DIG, Pegasus ou LaunchBox), especialmente em dispositivos com Android 7.0 ou inferior.
